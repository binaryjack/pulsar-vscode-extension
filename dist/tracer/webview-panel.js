...
exports.TracerWebviewProvider = void 0;
const node_buffer_1 = require("node:buffer");
const vscode = __importStar(require("vscode"));
class TracerWebviewProvider {
    ...
    async exportEvents() {
        ...
        await vscode.workspace.fs.writeFile(uri, node_buffer_1.Buffer.from(content, 'utf8'));
        vscode.window.showInformationMessage(`Exported ${this._events.length} events to ${uri.fsPath}`);
    }
    filterEvents(channels, types) {
        this._view?.webview.postMessage({
            type: 'filtered',
            events: this._events.filter((e) => (channels.length === 0 || channels.includes(e.channel)) &&
    ...
    _eventsToCSV() {
        ...
        const rows = this._events.map((e) => {
            ...
            const error = 'error' in e ? e.error.message : '';
    ...
    _getHtmlContent(webview) {
        const styleUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'media', 'tracer.css'));
        return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
exports.TracerWebviewProvider = TracerWebviewProvider;
TracerWebviewProvider.viewType = 'pulsar.tracerView';
